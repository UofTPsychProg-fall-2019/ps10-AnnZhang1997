...
# load in the data
setwd("~/GitHub/ps10-AnnZhang1997")
...
Q1 <- ggplot(ipip, aes(x=exer, y=BMI, color=gender)) +
    geom_boxplot(aes(fill=gender), outlier.shape=18) +
...
Q2a <- ggplot(ipip,aes(x=logMedInc,y=BMI, color=gender))+
    geom_point(size=.5,alpha=.4, shape=20)+
...
Q3 <- ggplot(ipip, aes(x=gender,y=logMedInc,color=exer, fill=exer, alpha=.5))+
    geom_bar(position='dodge', stat='summary', fun.y="mean") +
    stat_summary(fun.data = mean_cl_boot, conf = 0.95, geom = "errorbar", position=position_dodge(.9),
                 width=.4) +
...
Q4 <- ggplot(ipip.l, aes(x=BMI_cat, y=value, color=gender))+
    stat_summary(geom='pointrange', fun.data=mean_cl_boot, shape=20)+
...
ipip.g <- ipip.l %>% group_by(gender,BMI_cat,trait)%>%
...
Q5 <- ggplot(ipip.g, aes(x=BMI_cat, y=trait_mean, color=trait, group=trait))+
    geom_line(width=.3)+
    geom_point(shape=5)+
